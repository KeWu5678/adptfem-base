function ax = plotS1image(c4n, u, hmin)
%%PLOTS1IMAGE draws a  plot of an S1 function in current figure. It employs
%linear interpolation between nodal values. This functions works for
%rectangular domains only at the moment.
%   ax = plotS1image(c4n, u, hmin) gets the coordinates c4n, the
...
%
    ...
    %% FORMAT PLOT
    title(ax, {'Interpolated S1 image'; [num2str(size(c4n,1)), ' nodes']});
    colormap(ax, 'gray');
    colorbar(ax);
    axis('equal', 'tight');
    view(ax, 2);
